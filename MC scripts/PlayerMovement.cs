...
using UnityEngine;
//Скрипт работы ВСЕГО мувсета
//Тут уже будет сложно
...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (!IsJumping)
        {
            ...
            //Это нужно потому, что при повороте персонажа триггеры проверки стен отзеркаливаются вместе с персонажем
        ...
        //Вот не поверите, это - методы для анимации прыжков
        if(Input.GetKeyDown(KeyCode.X))
        //Это условие кардинально отличается по своей формулировке от условия проигрывания анимации падения, но на практике ничего бы не поменялось, если бы я выставил условие основанное на вертикальной скорости персонажа
        {
            anim.SetInteger("isFalling", 1); // Прыжок
        }
        else if(RB.velocity.y < -3) //Здесь это работает крайне странно и сейчас я разъясню почему
        #region Пояснение
        /*
        Дело в том, что юнити по каким-то неведомым
        причинам даже при простом движении по
        горизонтали задаёт игроку вертикальную скорость.
        Причём это произошло только после того,
        как я задал всем коллайдерам на уровне свойство kinematic в RigidBody.
        Скорость игроку задаётся примерно в 2.97,
        поэтому я поставил значение 3 в условии.
        */
        #endregion
        {
    ...
    {
        //Просчёт направления и скорости движения
    ...
    private void WallJump(int dir)
    {
        ...
        force.x *= dir; //приложение силы в противоположном направлении от стены
        ...
        //Тут импульс, потому что в противном случае будет игнорироваться масса, а нам это не надо
